...
export default function RTUTrackerApprovals() {
  ...
  // Removed handleRecheck and handleKeptForMonitoring - no longer used after removing Recheck and Kept for Monitoring buttons
    ...
    try {
      ...
      const recheckRemarks = recheckNote || 'Recheck requested by CCR';

      // Update approval status via approval API (include metadata with ccrStatus)
      const response = await fetch(`${API_BASE}/api/approvals/${approval._id}/status`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
      ...
      // Update RTU Tracker Site ccrStatus if rtuTrackerSiteId exists
      if (approval.rtuTrackerSiteId && ccrStatus) {
        const rtuTrackerSiteId = typeof approval.rtuTrackerSiteId === 'string' ? approval.rtuTrackerSiteId : approval.rtuTrackerSiteId._id;
        try {
          await fetch(`${API_BASE}/api/rtu-tracker-sites/${rtuTrackerSiteId}`, {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${token}`
          ...
          console.error('Error updating RTU Tracker Site ccrStatus:', rtuError);
        }
      }

      // Log recheck request action
      const row = approval.originalRowData || {};
      const siteCode = approval.siteCode || row['Site Code'] || row['SITE CODE'] || row['Code'] || row['CODE'] || '';
      const siteName = row['Site Name'] || row['SITE NAME'] || row['HRN'] || row['Hrn'] || row['Location'] || '';
      const approvalPhotos = photos(approval);
      ...
      logActivity({
        action: 'RTU Tracker Resolution Recheck Requested',
        typeOfIssue: approval.approvalType || 'RTU Tracker Resolution Approval',
        routingTeam: 'CCR Team',
        remarks: recheckRemarks,
        siteName: String(siteName),
  ...
  const handleKeptForMonitoring = async (approval: ApprovalRecord) => {
    ...
    try {
      ...
      const monitoringRemarks = recheckNote || 'Kept for monitoring by CCR';
      ...
      logActivity({
        action: 'RTU Tracker Resolution Kept for Monitoring',
        typeOfIssue: approval.approvalType || 'RTU Tracker Resolution Approval',
        routingTeam: 'CCR Team',
        remarks: monitoringRemarks,
        siteName: String(siteName),
